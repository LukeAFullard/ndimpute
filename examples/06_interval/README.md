# Example 06: Interval Censoring

This example demonstrates how to impute **interval-censored** data using Robust ROS with the Turnbull Estimator.

## What is Interval Censoring?

Interval censoring occurs when the exact value of an observation is not known, but it is known to lie within a specific range $(L, R]$.
*   **Exact Observation:** $L = R$ (e.g., `[10, 10]`)
*   **Left Censored:** $(0, LOD]$ (e.g., `[0, 0.5]`)
*   **Right Censored:** $[T, \infty)$ (e.g., `[100, inf]`)
*   **Interval Censored:** $(L, R]$ (e.g., known to be between 5 and 10)

## Method: Turnbull ROS

`ndimpute` handles this by:
1.  Using the **Turnbull Estimator** (Non-parametric Maximum Likelihood Estimator) to calculate the cumulative probability distribution (CDF) accounting for all overlapping intervals.
2.  Deriving **Plotting Positions** from the Turnbull CDF.
3.  Fitting a **Regression on Order Statistics (ROS)** model (Lognormal or Normal) to these points.
4.  Imputing values by sampling from the fitted distribution, truncated to the specific interval of each observation.

## Usage

Pass an `(N, 2)` array of bounds to `impute` with `censoring_type='interval'`.

```python
import numpy as np
from ndimpute import impute

# Data: [Left, Right] bounds
bounds = np.array([
    [10.0, 10.0],  # Exact
    [0.0, 5.0],    # Interval
    [20.0, np.inf] # Right Censored
])

df = impute(bounds, censoring_type='interval', method='ros')
```
